"""core/calendar — the Calendar boundary. The team's out-of-office truth lives
behind the CalendarClient interface; the chat/RAG app only ever touches this seam,
never a concrete backend. v1 backend is a local seeded mock (mock.py). A Google
CalendarClient adapter is a later drop-in that satisfies the same Protocol.

Ownership line: nothing about OOO is persisted in our Postgres — a tool reads a
transient team view through this interface and forgets it. Mirrors core/hris."""
from datetime import date
from typing import Protocol

from backend.core.tools.principal import Principal


class CalendarClient(Protocol):
    def whos_out(self, principal: Principal, start_date: date, end_date: date) -> list[dict]:
        """Colleagues whose out-of-office overlaps the inclusive ``[start_date,
        end_date]`` window, each as a display-safe ``{"name", "until"}`` row.

        The range is pushed to the backend so a real adapter (Google Calendar) can
        filter server-side rather than returning the whole calendar. The backend is
        responsible for the visibility projection — private fields (attendee emails,
        event titles, absence reason) must NOT be returned. Read-only team view;
        identity comes from the Principal, never LLM args."""
        ...
