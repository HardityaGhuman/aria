...
- "who is out/away this week" / "who is on leave right now" / "is anyone off today" -> calendar (needs the live team out-of-office list, not a document).
...
{user_message}"""
    ...
    if "calendar" in label:
        return "calendar"
    # `hr` last before the policy fallback: the other five labels contain no `hr`
