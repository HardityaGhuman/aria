...
| 7.5 | **Pre-deploy hardening** | ✅ pooling done | **DB connection pooling ✅ (2026-07-01).** Was: every query opened a fresh `psycopg.connect()` and relied on `with conn:` (transaction context — never closes), leaking connections until GC. Now: one process-wide `psycopg_pool.ConnectionPool` (`core/db.py`); each module's `_connect()` returns `db.pooled(error_factory)` (checks out, commits+returns on exit, maps conn-level failures to the typed error). Call sites unchanged. Closed on shutdown + test teardown. Size via `DB_POOL_MIN_SIZE/MAX_SIZE/MAX_IDLE` (1/10/300s). 153 tests green. Pairs with managed Postgres (Cloud SQL/Supabase) at deploy. |
| 8 | pgvector migration + Cloud Run deploy | ⏳ | Vectors off local Chroma into Postgres; the serverless cutover and hard hosting blocker. **EMBEDDING-DIM DECISION LOCKED (2026-07-01): keep local MiniLM at 384 dims for v1** → pgvector table is `vector(384)`, the Chroma→pgvector vector copy stays valid (no re-embed), Cloud Run runs `min-instances=1` to keep the 90MB model warm. Do NOT flip to API embeddings (`text-embedding-3-small` = 1536) in the same cut — that is a *different* dimension and forces a full corpus re-embed; it's a deliberate later step, not a free flag. Avoids the contradiction of "migrate 384 vectors + switch to 1536 model" → dimension mismatch. **Deploy bundle:** explicit `Dockerfile` (backend; bake the MiniLM weights), Cloud Run, managed Postgres + pgvector, secrets in Secret Manager, frontend as static build (Firebase Hosting / CDN), prod user seeding (strong per-account passwords, `COOKIE_SECURE=true`, real `FRONTEND_ORIGIN`). CI/CD via GitHub Actions. **Build the vector-store abstraction seam (Protocol, like `SessionStore`) at the START of this step, designed against pgvector's real query signature — not pre-built now (the second impl defines the interface; guessing it from Chroma alone risks the wrong seam).** |
